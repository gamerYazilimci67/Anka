...
""")
    ...
    def save_html(self, html):
        file_name, _ = QFileDialog.getSaveFileName(self, "Save", "", "HTML File (*.html);;WEBP File(*.webp);;All Files (*)")
        if file_name:
    ...
    def bookmark(self):
        ...
        with open(bookmarks, 'r', encoding="utf-8") as bookmarks_file:
            a = bookmarks_file.read()
            if current_url in a:
                pass
            else:
                old_bookmarks = bookmarks_file.read()
                new_bookmarks = current_url + "\n" + old_bookmarks
                with open(bookmarks,'w', encoding="utf-8") as bookmarks_file2:
                  bookmarks_file2.write(new_bookmarks)
...
class AnkaBrowserSettings(QDialog):
    def __init__(self, parent=None):
        ...
        self.note_label = QLabel("Please restart the browser for configure the settings.")
